...
@IBDesignable class LikeControl: UIControl {
    ...
    @IBInspectable var count: Int = 99 {
        didSet {
            update()
        }
    }
    
    @IBInspectable var iconOff: UIImage = UIImage(systemName: "heart")! {
        didSet {
            update()
        }
    }
    
    @IBInspectable var iconOn: UIImage = UIImage(systemName: "heart.fill")! {
        didSet {
            update()
        }
    }
...
//        button.sizeToFit()
    ...
    @objc func like(_ sender: UIButton) {
        ...
        if !status {
            ...
            sender.setImage(iconOn, for: .normal)
        } else {
            count -= 1
            sender.tintColor = unactiveColor
            sender.setImage(iconOff, for: .normal)
        }
        sender.setTitle(" " + String(count), for: .normal)
