...
class FriendsTableViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, FilterControlDelegate {
    ...
    @objc func refresh() {
        ...
        friends = loadUsersData(amount: 15)
...
//        return cell
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard let photosView = segue.destination as? PhotosViewController else { return }
        let indexPath = tableView.indexPathForSelectedRow!
        let cell = tableView.cellForRow(at: indexPath) as! FriendTableViewCell
        for friend in friends {
            if friend.name == cell.nameLabel.text {
                photosView.photos = []
                allPhotos.insert(friend.avatar.first!!, at: 0)
                let originalAllPhotos = Array(Set(allPhotos))
                photosView.photos.append(contentsOf: originalAllPhotos)
                break
            }
        }
