...
class MeViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if nameLabel.text == "Name Surname" {
            loadData()
        } else {
            loading.endLoad()
    ...
    func loadData() {
        ...
        avatatImage.image = LoremIpsum.placeholderImage(from: .loremPixel, with: CGSize(width: 100, height: 100))
        nameLabel.text = LoremIpsum.name()
        for _ in 1...5 {
            photos.append(LoremIpsum.placeholderImage(from: .loremPixel, with: CGSize(width: Int.random(in: 200...500), height: Int.random(in: 100...300))))
        }
