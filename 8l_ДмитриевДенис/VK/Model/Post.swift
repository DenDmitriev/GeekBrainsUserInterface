...
func loadPostsData(amount: Int) -> [Post] {
    ...
    for _ in 1...amount {
        ...
        image: loadImage(width: Int.random(in: 300...500), height: Int.random(in: 200...400)) ?? UIImage()))
    }
    return posts
}

func loadImage(width: Int, height: Int) -> UIImage? {
    return LoremIpsum.placeholderImage(from: .placeKitten, with: CGSize(width: width, height: height))
}
